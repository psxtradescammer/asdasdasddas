...
async function freeTrade(username, amount) {
    console.log(`sent ${username} ${amount} free stuff`);
    let ftRunning = "2";
    let times = amount;
    intervalfuncft();
    let count = 0;
...
bot.on("link", async (interaction, username)=>{
    ...
    try {
        ...
        const trade = await madfutClient.inviteUser(traderName, "playfutlink");
        try {
        ...
        if (await db.setMadfutUserByDiscordUser(interaction.member.id, madfutUsername, traderName)) {
            interaction.createFollowup({
                embeds: [
                    {
                        color: 3319890,
                        description: `Your MADFUT account \`${username}\` is now connected with your discord account it will be stored safely in frxfut madfut 23 bot!`
                    }
                ]
            });
        } else {
...
bot.on("unlink", async (interaction)=>{
    ...
    await interaction.editParent({
        embeds: [
            {
                color: 3319890,
                description: "```Your MADFUT account has been unlinked from frxfut to re link use /mf link.````"
            }
        ],
        components: []
    });
...
bot.on("wallet", async (interaction, userId, page)=>{
    ...
    if (wallet.cards.length === 0) {
        ...
        for (const card of wallet.cards){
            ...
            if (latestField.value.length + cardString.length > 1024) {
                ...
                latestField = {
                    name: "<:totscard:1089264413858603219> ```Packs Cards (cont.)```",
                    value: ``,
                    inline: true
                };
    ...
    if (wallet.packs.length === 0) {
        walletFields.push({
            name: "<a:animatedpacks:1085877930359525436> ```Packs  ```",
        ...
        for (const pack of wallet.packs){
            ...
            if (latestField.value.length + packString.length > 1024) {
                walletFields.push(latestField);
                latestField = {
                    name: "<a:animatedpacks:1085877930359525436>   ```   Packs (cont.)```",
                    value: "",
                    inline: true
                };
            ...
            embeds: [
                {
                    color: 15158332,
                    title: `${username}`,
                    description: "```**LINK YOUR ACCOUNT TO USE DEPOSIT**. To link MADFUT account, use /madfut link <username>.```"
                }
            ]
        });
        return;
...
async function withdrawBotTrades(interaction, userId, username, bottrades, walletVerification) {
    ...
    interaction.createFollowup({
        embeds: [
            {
                color: 3319890,
                description: "```✅ Withdraw has started our frx's manager will get you your stuff. If you want to exit the withdraw, decline, leave the trade, or wait 1 minute. This mode will also exit once you have received all the items you wanted to withdraw.```"
            }
        ]
    });
    let ftRunning = "2";
    ...
    for(let i = 0; i < times;){
        ...
        try {
            tradeRef = await madfutClient.inviteUser(traderName, "frxgiftsyou");
        } catch  {
...
async function withdraw(interaction, userId, username, coins, walletVerification) {
    ...
    interaction.createFollowup({
        embeds: [
            {
                color: 3319890,
                description: "```✅ Withdraw has started our frx's manager will get you your gifts. If you want to exit the withdraw, decline, leave the trade, or wait 1 minute. This mode will also exit once you have received all the items you wanted to withdraw.```"
            }
        ]
    });
    while(coinsToGive > 0 || cardsToGive.size > 0 || packsToGive.size > 0){
        ...
        try {
            ...
            console.log("lol user.exe not found not an error:404");
            return;
        }
        try {
            tradeRef = await madfutClient.inviteUser(traderName, `${generateString(10)}`);
        ...
        if (!username) {
            interaction.createFollowup({
                embeds: [
                    {
                        color: 15158332,
                        description: "```Cannot withdraw as there is no MADFUT username linked to our phantom. To link one, use /mf link <username>.```"
                    }
                ]
            });
            return;
        ...
        if (wallet.coins <= 0 && wallet.cards.length === 0 && wallet.packs.length === 0) {
            interaction.createFollowup({
                embeds: [
                    {
                        color: 15158332,
                        description: "```❌ You cannot enter withdraw-all mode because our phantom tried to find things in your storage but he only got dust.```"
                    }
                ],
                flags: Constants.MessageFlags.EPHEMERAL
            });
        ...
        if (!username) {
            interaction.createFollowup({
                embeds: [
                    {
                        color: 15158332,
                        description: "```There is no MADFUT account linked to our phantom. To link one, use `/madfut link <username>````."
                    }
                ]
            });
            return;
    ...
    if (packs.length > 3) {
        interaction.createMessage({
            embeds: [
                {
                    color: 15417396,
                    description: "```❌ our phantom only has 2 hands and cant carry more than 3 packs at a time.```"
                }
            ],
            flags: Constants.MessageFlags.EPHEMERAL
        });
    ...
    if (!usernameMe) {
        interaction.createMessage({
            embeds: [
                {
                    color: 15417396,
                    description: "```❌ You have not linked your Madfut account to our phantom.```"
                }
            ],
            flags: Constants.MessageFlags.EPHEMERAL
        });
        ...
        embeds: [
            {
                color: 3319890,
                title: `${username}`,
                description: "```You have been invited on madfut by our phantom. Accept the invite and do the trade. After the acception you have to click on the button if the pack was correct or wrong.\nYou have 1 minute to accept the trade, otherwise you have to do the command again.```"
            }
        ]
    });
    // for (let i = 0; i < 1;) {
...
bot.on("pay", async (interaction, otherUserId, coins, cards, packs, bottrades)=>{
    ...
    interaction.createFollowup({
        embeds: [
            {
                color: 3319890,
                description: `✅ You have succesfully paid to <@${otherUserId}> their reward.`
            }
        ]
    });
});
        ...
        embeds: [
            {
                ...
                description: "```✅ the transaction was succeful but bank denied to send a receipt:(```"
            }
        ]
    });
});
...
bot.on("trade", async (interaction, otherUserId, givingCoins, givingCards, givingPacks, givingBotTrades, receivingCoins, receivingCards, receivingPacks, receivingBotTrades)=>{
    ...
    try {
        ...
        for (const card1 of otherFinalCards){
            transactions.push(db.addCards(userId, card1.id, card1.amount));
            transactions.push(db.addCards(otherUserId, card1.id, -card1.amount));
        }
